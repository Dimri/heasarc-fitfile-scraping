...
from utils import angularDistance
...
def angle_to_grb(ra, dec, trigdat_file, verbose=False):
    ...
    val_ang = []
    for d in det:
        angle = angularDistance.getDetectorAngle(
            ra_scx, dec_scx, ra_scz, dec_scz, ra_obj, dec_obj, d
        )
        angls[round(angle)] = d
    ...
    if verbose:
        ...
        print(
            "The brightest 3 NaI detectors are",
            angls_nai[res_nai[0]],
            "(",
            res_nai[0],
            "deg)",
